...
2. **Bước 2 (User -> Client):** Chọn một trong ba nút hành động:
  ...
  3. Hộp thoại lịch hiện lên -> Người dùng chọn thời điểm mới (Ví dụ: báo lại sau 1 tiếng hoặc dời sang ngày mai).
  4. Gửi request cập nhật. API thực hiện chỉnh sửa cột `snoozed_until` hoặc thay đổi trực tiếp ngày đến hạn:
    ```sql
     UPDATE care_reminder_logs 
  ...
  5. Gửi lệnh xóa lên API -> API thu hồi nhắc nhở hiện tại và tắt lịch nhắc nhở:
    ```sql
